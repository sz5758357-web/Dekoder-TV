...
const channelsEl = document.getElementById('channels'),
  ...
  welcome = document.getElementById('welcome'), // Karta powitalna
...
function render(list) {
  ...
  list.forEach((ch, i) => {
    ...
    el.innerHTML = `<div class='dot'></div><div class='num'>${ch.num\vert{}\vert{}(i+1)}</div><div class='name'>${ch.name}</div><div class='fav'>★</div>`;
    el.onclick = () => {
      current = i;
      render(filtered);
      play(ch);
...
function showInfo(ch) {
  infoContent.innerHTML = `<div style="font-weight:700;margin-bottom:4px;">${ch.name||'Kanał'}</div><div style='color:var(--muted);font-size:12px;word-break:break-all;'>${ch.url\vert{}\vert{}''}</div><div style='color:var(--muted);font-size:12px;margin-top:4px;'>Ulubiony:${favs.includes(ch.url)?'tak':'nie'}</div>`;
...
// --- ZMIANA: Nowa funkcja pokazująca ekran powitalny ---
function showWelcome() {
    started = false;
    welcome.classList.remove('hidden');
    nowName.textContent = 'Witaj w HypeDekoder';
    nowInfo.textContent = 'Instrukcje obsługi';
    if (hls) { hls.destroy(); hls = null; }
    video.src = ''; // Zresetuj źródło wideo
}

function play(ch) {
  if (!ch) return;
  started = true;
  welcome.classList.add('hidden'); // Ukryj ekran powitalny po kliknięciu kanału
...
async function loadJSONText(text) {
  ...
  applyFilter();
  // --- ZMIANA: Nie odtwarzaj automatycznie, pokaż powitanie ---
  showWelcome();
}
...
// --- ZMIANA: Kliknięcie w obszar okna (poza panelem bocznym) aktywuje aplikację (ukrywa powitanie) ---
document.addEventListener('click', (e) => {
  if (!started && e.target.closest('.viewer')) {
    welcome.classList.add('hidden');
    started = true;
    // Opcjonalnie odtwórz pierwszy kanał po kliknięciu
...
// --- ZMIANA: Klawisz spacji lub Enter również aktywuje aplikację ---
document.addEventListener('keydown', e => {
    if (!started && [' ', 'Enter'].includes(e.key)) {
        e.preventDefault(); // Zapobiegaj przewijaniu strony spacją
        welcome.classList.add('hidden');
        started = true;
        if (filtered.length > 0) {
            current = 0;
    ...
    // Reszta nawigacji
    if (!started) return; // Ignoruj klawisze jeśli nie zacząłeś
    ...
    if (e.key === 'ArrowDown') {
        ...
        if (current < 0) current = 0;
        render(filtered);
        if (filtered[current]) play(filtered[current]); // Odtwórz podgląd
    }
    if (e.key === 'ArrowUp') {
...
function playLast() {
    // --- ZMIANA: Zawsze pokazuj powitanie na start ---
    if (autostart && lastChannel && lastChannel.url) {
        channels = [lastChannel];
        filtered = [lastChannel];
        render(filtered);
        // Opcjonalnie: autostart działa, ale nie pokazuj powitania
        // play(lastChannel);
        // LUB: pokazuj powitanie nawet przy autostarcie, żeby użytkownik wiedział co robić
        showWelcome();
    } else {
        showWelcome();
    }
